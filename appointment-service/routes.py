...
def create_appointment():
    ...
    # Forward the X-Patient-ID header from the incoming request
    x_patient_id = request.headers.get("X-Patient-ID")
    if not x_patient_id:
        return jsonify({"error": "Missing X-Patient-ID header"}), 400

    patient_headers = {
        "X-Patient-ID": x_patient_id
    }

    # Validate patient exists
    patient_response = requests.get(
        f"http://patient-service:5001/patients/{data['patient_id']}",
        headers=patient_headers
    )

    # Validate doctor exists (no header needed)
    doctor_response = requests.get(
        f"http://doctor-service:5002/doctors/{data['doctor_id']}"
    )
